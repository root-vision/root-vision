...
const SOCIALS = [
  ...
  { platform:"THE WATCHER",handle:"",                  desc:"Tizimni ichidan ko'radi. Hech kim uchun ishlamaydi. Hech kimga bog'liq emas. U — erkin.",                   link:"",                                              Logo:InstagramSVG,accent:"#cc0000", bg:"rgba(0,0,0,0)",          border:"rgba(180,0,0,0.12)" },
];
...
const Z_GAP   = 900;
const CAM_SPD = 2.6;
const N_ITEMS = SOCIALS.length + 1; // cards + ROOT VISION
...
export default function Home() {
  ...
  // Per-card spring X — smooth side-to-center motion
  const cardSX  = useRef(SOCIALS.map(() => 0)); // current X
  const cardTX  = useRef(SOCIALS.map(() => 0)); // target X
  const cardVX  = useRef(SOCIALS.map(() => 0)); // velocity X
  const cardSY  = useRef(SOCIALS.map(() => 0)); // current Y
  const cardTY  = useRef(SOCIALS.map(() => 0)); // target Y
  const cardVY  = useRef(SOCIALS.map(() => 0)); // velocity Y
  const cardSR  = useRef(SOCIALS.map(() => 0)); // current rotation
  const cardTR  = useRef(SOCIALS.map(() => 0)); // target rotation
  const cardVR  = useRef(SOCIALS.map(() => 0)); // velocity rotation
  const [cardPositions, setCardPositions] = useState(
    SOCIALS.map(() => ({x:0, y:0, r:0}))
  );
  ...
  useEffect(() => {
    ...
    const loop = (now:number) => {
      ...
      // Spring physics for each card X/Y/rotation
      const W2 = window.innerWidth;
      const newPos = SOCIALS.map((_, i) => {
        const baseZ = -i * Z_GAP;
        let relZ = baseZ + scrollSmooth.current * CAM_SPD;
        relZ = ((relZ % LOOP) + LOOP) % LOOP;
        if (relZ > 800) relZ -= LOOP;

        const isWatcher  = SOCIALS[i]?.platform === "THE WATCHER";
        const waitSide   = i % 2 === 0 ? 1 : -1;
        const sideWait   = isWatcher ? 0 : waitSide * W2 * 0.36;
        const onDeckStart = -Z_GAP * 0.88;
        const onDeckEnd   = -Z_GAP * 0.12;

        let tx = 0, ty = (seeded(i*7)-0.5)*40, tr = 0;

        if (relZ < onDeckStart) {
          tx = sideWait * 0.55;
          tr = isWatcher ? 0 : waitSide * 7;
        } else if (relZ < onDeckEnd) {
          const t = (relZ - onDeckStart) / (onDeckEnd - onDeckStart);
          tx = sideWait * (1 - t * 0.1);
          tr = isWatcher ? 0 : waitSide * (7 - t * 7);
        } else if (relZ < 0) {
          const t = relZ / onDeckEnd; // 1→0 as relZ goes onDeckEnd→0
        ...
        // Spring: stiffness 0.055, damping 0.72
        const stiff = 0.055, damp = 0.72;
        cardVX.current[i] += (cardTX.current[i] - cardSX.current[i]) * stiff;
        cardVX.current[i] *= damp;
        cardSX.current[i] += cardVX.current[i];

        cardVY.current[i] += (cardTY.current[i] - cardSY.current[i]) * stiff;
        cardVY.current[i] *= damp;
        cardSY.current[i] += cardVY.current[i];

        cardVR.current[i] += (cardTR.current[i] - cardSR.current[i]) * stiff;
        cardVR.current[i] *= damp;
        cardSR.current[i] += cardVR.current[i];

        return { x: cardSX.current[i], y: cardSY.current[i], r: cardSR.current[i] };
      });
  ...
  return (
    <>
      ...
      <div style={{position:"fixed",inset:0,background:"#000",overflow:"hidden"}}>
        ...
        <div style={{position:"absolute",inset:0,zIndex:5,perspective:`${fov}px`,overflow:"hidden"}}>
          <div style={{position:"absolute",top:"50%",left:"50%",transformStyle:"preserve-3d",transform:`rotateX(${tiltX}deg)`,willChange:"transform"}}>
            ...
            {mounted && SOCIALS.map((card,i)=>{
              ...
              // Only show ONE card waiting — hide anything beyond one slot back
              const visible = relZ > -(Z_GAP * 1.05);
              if (!visible) return null;

              const {x:xOff, y:yOff, r:rot} = cardPositions[i];

              return (
                <div key={i} style={{
                  position:"absolute",
                  opacity:alpha,
                  transform:`translate3d(${xOff}px,${yOff}px,${relZ}px) rotateZ(${rot}deg)`,
                  willChange:"transform,opacity",
                }}>
                  <a className="card-link" href={card.link||"#"} target={card.link?"_blank":"_self"} rel="noopener noreferrer" style={{"--ca":card.accent,"--cg":card.accent+"33"} as React.CSSProperties}>
                    {card.platform === "THE WATCHER" ? (
                      /* ── WATCHER special card ── */
                      <div className="cbox" style={{width:310,height:420,background:"rgba(0,0,0,0.85)",border:"1px solid rgba(180,0,0,0.18)",backdropFilter:"blur(4px)",transform:"translate(-50%,-50%)",position:"relative",overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center"}}>
                        <div style={{position:"absolute",top:-1,left:-1,width:14,height:14,borderTop:"1.5px solid rgba(180,0,0,0.6)",borderLeft:"1.5px solid rgba(180,0,0,0.6)"}}/>
                        <div style={{position:"absolute",bottom:-1,right:-1,width:14,height:14,borderBottom:"1.5px solid rgba(180,0,0,0.6)",borderRight:"1.5px solid rgba(180,0,0,0.6)"}}/>
                        {/* Personaj rasmi — mix-blend-mode:screen oq qismlarni yo'q qiladi */}
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img
                          src="/personaj_oqnursiz.png"
                          alt="The Watcher"
                          style={{
                        ...
                        <div style={{position:"absolute",bottom:"1.5rem",left:0,right:0,textAlign:"center",zIndex:2}}>
                          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.1rem",letterSpacing:"0.35em",color:"rgba(180,0,0,0.7)",textTransform:"uppercase"}}>Root Vision</div>
                          <div style={{fontFamily:"'Inter',sans-serif",fontWeight:300,fontSize:"0.55rem",letterSpacing:"0.4em",color:"rgba(140,0,0,0.5)",marginTop:"0.3rem",textTransform:"uppercase"}}>The Watcher</div>
                        </div>
                      </div>
                    ) : (
                    <div className="cbox" style={{width:310,height:420,background:card.bg,border:`1px solid ${card.border}`,backdropFilter:"blur(12px)",padding:"1.8rem",display:"flex",flexDirection:"column",justifyContent:"space-between",transform:"translate(-50%,-50%)",position:"relative",boxShadow:`0 0 28px ${card.accent}15`}}>
                      ...
                      <div>
                        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:`1px solid ${card.border}`,paddingBottom:"0.8rem",marginBottom:"1rem"}}>
                          <card.Logo/>
                          <span style={{fontFamily:"'Inter',sans-serif",fontWeight:300,fontSize:"0.55rem",letterSpacing:"0.38em",color:"rgba(255,255,255,0.22)",textTransform:"uppercase"}}>{String(i+1).padStart(2,"0")} / {SOCIALS.length}</span>
                        </div>
            ...
            {/* ROOT VISION in loop — slot 5 — rendered in 3D for positioning only, no blur here */}
            {mounted&&(()=>{
              const baseZ = -SOCIALS.length*Z_GAP;
              let relZ = baseZ+cameraZ;
              relZ = ((relZ%LOOP)+LOOP)%LOOP;
              if(relZ>800) relZ-=LOOP;
              const alpha = relZ<-4000?0
                :relZ<-2200?(relZ+4000)/1800
                :relZ>-100?Math.max(0,1-(relZ+100)/350)
                :1;
              if(alpha<=0) return null;
              // Only render when in front (no blur needed)
              if(relZ < -300) return null;
              return (
        ...
        {/* ROOT VISION — blurred ghost layer (outside 3D, so filter works) */}
        {mounted&&(()=>{
          const baseZ = -SOCIALS.length*Z_GAP;
          let relZ = baseZ+cameraZ;
          relZ = ((relZ%LOOP)+LOOP)%LOOP;
          if(relZ>800) relZ-=LOOP;
          // Only show when far back
          if(relZ >= -300) return null;
          const alpha = relZ<-4000?0:relZ<-2200?(relZ+4000)/1800:1;
          if(alpha<=0.01) return null;
          const blurPx = Math.min(18, (-relZ-300)/90);
          // Scale down as it goes further
          const sc = Math.max(0.3, 1+relZ/3000);
          return (
            <div style={{
